...
export default function ProfileClient({ registrationNo, initialTab = "overview" }) {
  ...
  return (
    <main className="p-4 h-full bg-card">
      {/* Breadcrumbs */}
          ...
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            ...
            <div className="flex flex-col sm:items-end sm:justify-end pt-21">
              <div className="flex gap-2">
                <button onClick={() => setTab("attendance")} className="inline-flex items-center justify-center rounded-md border-2 border-primary bg-primary px-3 py-2 text-sm text-white hover:opacity-90 shadow-sm hover:shadow-md">
                  View attendance
                </button>
                <button onClick={()=> makeStudentCR(s.registration_no, fullName)} className="rounded-md border border-primary bg-primary px-3 py-2 text-sm text-white hover:opacity-90 shadow-sm hover:shadow-md">
                  Promote to CR
                </button>
          ...
          {["overview", "attendance"].map((t) => (
            <button
              ...
              className={`-mb-px border-b-2 px-6 py-2 text-md capitalize hover:text-foreground hover:bg-card hover:border-b-1 hover:border-foreground hover:shadow-sm ${
                tab === t ? "border-foreground text-foreground bg-card shadow-md" : "border-transparent text-muted-foreground  "
              }`}
            >
              {t}
            </button>
          ...
          <div className="rounded-lg border bg-card p-4  shadow-sm hover:shadow-lg">
            ...
            {loading ? (
              ...
              <ul className="space-y-2 text-sm">
                <li><span className="text-muted-foreground">Phone:</span> {s.contact_number ?? "—"}</li>
                <li><span className="text-muted-foreground">Email:</span> {s.email ?? "—"}</li>
                <li><span className="text-muted-foreground">Guardian:</span> {s.guardian_email ?? "—"}</li>
              </ul>
            ) : null}
          </div>
        </section>
