...
export default function SubjectDetailsPage() {
  ...
  const [rosterViewMode, setRosterViewMode] = useState<"auto" | "table" | "grid">("auto");
  const [showAllRosterCards, setShowAllRosterCards] = useState(false);
  ...
  const rosterColumns: Column<NonNullable<SubjectFullDetails["student_roster"]>[0]>[] = [
    {
      header: "Student Name",
      accessor: (st) => (
        <span className="font-bold text-foreground block text-center">{st.student_name}</span>
      ),
    },
    {
      header: "Registration No",
      accessor: (st) => (
        <span className="font-mono font-black text-xs sm:text-sm text-indigo-600 dark:text-indigo-400">
          {st.registration_no}
        </span>
      ),
    },
    ...
    },
    {
      ...
      accessor: (st) => (
        <span
          className={`font-black text-xs sm:text-sm ${st.attendance_pct >= 75
    ...
    },
    {
      ...
      accessor: (st) => {
        ...
        if (pct >= 75) {
          return (
            <span className="inline-flex items-center gap-1.5 rounded-lg bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 border border-emerald-500/20 px-2.5 py-1 text-[11px] font-extrabold">
              ✓ Healthy
            </span>
          );
        } else if (pct >= 60) {
          return (
            <span className="inline-flex items-center gap-1.5 rounded-lg bg-blue-500/10 text-blue-600 dark:text-blue-400 border border-blue-500/20 px-2.5 py-1 text-[11px] font-extrabold">
              ⚡ Moderate
            </span>
          );
        } else if (pct >= 40) {
          return (
            <span className="inline-flex items-center gap-1.5 rounded-lg bg-amber-500/10 text-amber-600 dark:text-amber-400 border border-amber-500/20 px-2.5 py-1 text-[11px] font-extrabold">
              ⚠️ Warning
            </span>
          );
        } else {
          return (
            <span className="inline-flex items-center gap-1.5 rounded-lg bg-rose-500/10 text-rose-600 dark:text-rose-400 border border-rose-500/20 px-2.5 py-1 text-[11px] font-extrabold">
              🚨 Critical
            </span>
          );
        }
  ...
  const sessionColumns: Column<NonNullable<SubjectFullDetails["sessions"]>[0]>[] = [
    {
    ...
    },
    {
      header: "Conducted By",
      accessor: (sess) => (
        <span className="font-semibold text-foreground">{sess.submitted_by}</span>
      ),
    },
    {
          ...
          <div className="space-y-5">
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                  <div>
                    <h3 className="text-sm sm:text-base font-extrabold text-foreground flex items-center gap-2">
                      <span>📈 Session Attendance Trend</span>
                    </h3>
                    <p className="text-xs text-muted-foreground">
                ...
                {trendData.length === 0 ? (
                  ...
                  <div className="space-y-4 pt-2">
                    ...
                    <div className="flex items-center justify-between text-[11px] font-semibold text-muted-foreground px-1">
                      <span className="text-indigo-600 dark:text-indigo-400 font-bold sm:hidden flex items-center gap-1">
                        <span>👈 Swipe graph horizontally 👉</span>
                      </span>
                      <span className="hidden sm:inline">← Oldest Session</span>
                      <span className="hidden sm:inline">Most Recent Session →</span>
                    </div>

                    {/* Smooth Touch-Scrollable SVG Trend Chart Container */}
                    <div className="w-full overflow-x-auto touch-pan-x rounded-2xl border border-border/60 bg-muted/20 p-2 relative">
                      <div className="min-w-[540px] sm:min-w-full h-48 sm:h-56 relative">
                                ...
                                {pts.map((pt) => {
                                  ...
                                  return (
                                    <g key={pt.item.session_id + pt.i}>
                                      {/* Wide Invisible Hover Target Hitbox */}
                                      <rect
                                        x={pt.x - stepWidth / 2}
                                        y="0"
                                        width={stepWidth}
                                        ...
                                        onClick={() => setHoveredTrendPoint(hoveredTrendPoint === pt.i ? null : pt.i)}
                                      />
                        ...
                        {hoveredTrendPoint !== null && trendData[hoveredTrendPoint] && (() => {
                          ...
                          return (
                            <div className="absolute top-2 left-1/2 -translate-x-1/2 z-30 bg-slate-900 text-slate-100 dark:bg-slate-800 dark:text-slate-100 border border-slate-700 px-3.5 py-2 rounded-xl shadow-xl whitespace-nowrap animate-in fade-in zoom-in-95 duration-150 flex flex-col items-center justify-center text-center pointer-events-none gap-0.5 max-w-[90%]">
                              <div className="flex items-center gap-2 font-mono text-xs">
                                <span className="font-extrabold">
                                  {activeItem.present_count} / {activeSize} <span className="text-[10px] font-sans text-slate-400 font-bold uppercase">Students</span>
                                </span>
                                <span className="text-slate-500 font-sans">|</span>
                                <span className="font-extrabold text-emerald-400 bg-emerald-500/20 px-1.5 py-0.5 rounded border border-emerald-500/30 text-[11px]">
                                  {activePct}%
                                </span>
                              </div>
            ...
            <div className="solid-card rounded-2xl border border-border overflow-hidden bg-card space-y-4 p-5">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-border pb-4">
                <div>
                  <h3 className="text-base font-extrabold text-foreground">
                    Enrolled Student Attendances ({details.stats.enrolled_students_count})
                  </h3>
                  <p className="text-xs text-muted-foreground">
                ...
                <div className="flex flex-col sm:flex-row sm:items-center gap-3">
                  {/* View Mode Toggle */}
                  <div className="flex items-center gap-1.5 bg-muted p-1 rounded-xl border border-border self-start sm:self-end">
                    <button
                      type="button"
                      ...
                      className={`px-2.5 py-1 rounded-lg text-xs font-bold transition-all ${rosterViewMode === "table"
                        ...
                        : rosterViewMode === "grid"
                          ...
                          : "text-muted-foreground hover:text-foreground sm:bg-card sm:text-foreground sm:shadow-xs sm:border sm:border-border"
                        }`}
                    >
                      Table View
                    </button>
                    <button
                      ...
                      className={`px-2.5 py-1 rounded-lg text-xs font-bold transition-all ${rosterViewMode === "grid"
                        ...
                        : rosterViewMode === "table"
                          ? "text-muted-foreground hover:text-foreground"
                          : "bg-card text-foreground shadow-xs border border-border sm:bg-transparent sm:text-muted-foreground sm:border-transparent sm:shadow-none"
                        }`}
                    >
                      Cards View
                    </button>
              ...
              {filteredRoster.length === 0 ? (
                ...
                <>
                  {/* Roster Cards View (Default 5 students initially with Show All toggle) */}
                  <div
                    className={
                      rosterViewMode === "grid"
                        ? "grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4 pt-2"
                        : rosterViewMode === "table"
                          ? "hidden"
                          : "grid grid-cols-1 gap-3 sm:hidden pt-2"
                    }
                  >
                    {(showAllRosterCards ? filteredRoster : filteredRoster.slice(0, 5)).map((st) => (
                      <div
                    ...
                    {filteredRoster.length > 5 && (
                      <div className="col-span-full text-center pt-2">
                        <button
                          ...
                          className="inline-flex items-center gap-1.5 px-4 py-2 rounded-xl text-xs font-bold text-indigo-600 dark:text-indigo-400 bg-indigo-500/10 hover:bg-indigo-500/20 border border-indigo-500/20 transition-all active:scale-95"
                        >
                          <span>
                            {showAllRosterCards
                              ? "Show Less (Top 5 Students)"
                              : `Show All (${filteredRoster.length} Students)`}
                          </span>
                          <span>{showAllRosterCards ? "↑" : "↓"}</span>
                        </button>
                      </div>
                    )}
                  </div>

                  {/* Roster Table View (Default on desktop >= 640px or when explicitly selected) */}
                  <div
                    className={
                      rosterViewMode === "table"
                        ? "block"
                  ...
                  >
                    <DataTable
                      ...
                      textsize="text-xs"
                      onRowClick={(item) =>
                        router.push(`/faculty/get-student-by-id?reg=${encodeURIComponent(item.registration_no)}`)
                      }
                    />
                  </div>
                </>
            ...
            <div className="solid-card rounded-2xl border border-border p-5 bg-card space-y-4">
              ...
              {details.sessions.length === 0 ? (
                ...
                <>
                    ...
                    {(showAllSessions ? details.sessions : details.sessions.slice(0, 5)).map((sess) => (
                      <div key={sess.session_id} className="p-3.5 rounded-xl border border-border bg-muted/40 space-y-2">
                        ...
                        <div className="flex items-center justify-between text-xs">
                          <span className="text-muted-foreground font-medium">Conducted By:</span>
                          <span className="font-semibold text-foreground">{sess.submitted_by}</span>
                        </div>
                  ...
                  {/* Desktop Table View for Recent Sessions Log */}
                  <div className="hidden sm:block">
                    <DataTable
                      data={showAllSessions ? details.sessions : details.sessions.slice(0, 5)}
                  ...
                  {/* Show All / Show Less Button for Sessions Log */}
                  {details.sessions.length > 5 && (
                    <div className="text-center pt-2">
                      ...
                      >
                        <span>
                          {showAllSessions
                            ? "Show Less (Recent 5 Sessions)"
                            : `Show All (${details.sessions.length} Sessions Logged)`}
                        </span>
