  ...
  const tableColumns: Column<Student>[] = [
    {
      ...
      accessor: (item) => (
        <span className="font-mono text-sm sm:text-base font-black text-indigo-600 dark:text-indigo-400 tracking-wide">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-xs sm:text-sm text-muted-foreground">
            Browse and search student rosters filtered by department and semester. Click any card to open detailed profile analytics.
          </p>
        </div>
          ...
          {filteredStudents.map((s) => (
            ...
            >
              ...
              <div className="pt-3 border-t border-border/60 flex items-center justify-between gap-2 text-xs font-bold">
                ...
                {s.roll_number && (
                  <span className="text-[10px] font-extrabold text-muted-foreground/70 bg-muted px-2 py-0.5 rounded-md border border-border">
                    Roll: {s.roll_number}
                  </span>
                )}
              </div>
