...
const variantStyles: Record<BadgeVariant, { bg: string; dot: string }> = {
  ...
  warning: {
    bg: "bg-amber-50 text-amber-700 dark:bg-amber-950/60 dark:text-amber-400 border-amber-200 dark:border-amber-800",
...
export function Badge({ children, variant = "primary", pulse = false, className = "" }: BadgeProps) {
  ...
  return (
    <span
      className={`inline-flex items-center gap-1.5 rounded-full border px-2.5 py-0.5 text-[11px] font-bold tracking-wide transition-colors duration-150 ${style.bg} ${className}`}
    >
